...
`drug_names` for the names of each intervention in the dataset. To obtain the 
network plot, `netplot` calls the `nma.networkplot` function from the 
**pcnetmeta** package.
...
of their project by adding the argument `save_xls = TRUE` in the `netplot` 
function.
...
Network Meta-Analysis in R with the pcnetmeta Package. *J Stat Softw* 2017;
**80**(5):1--25. 
[doi: 10.18637/jss.v080.i05](https://www.jstatsoft.org/article/view/v080i05)
